...
* 其中默认输出为稀疏分布（忽略先验的作用），如需获得考虑先验参数的稠密主题分布需调用dense_topic_dist函数接口。
...
程序默认采用短文本与长文本语义匹配计算模式，运行结果如下所示
    ...
    请输入长文本:
    百度又一次展示了自动驾驶领域领导者的大气风范，发布了一项名为“Apollo(阿波罗)”的新计划，向汽车行业及自动驾驶领域的合作伙伴提供一个开放、完整、安全的软件平台，帮助他们结合车辆和硬件系统，快速搭建一套属于自己的完整的自动驾驶系统。
    Query Tokens: 百度 阿波罗 计划 开放 驾驶 技术 有望 改变 汽车 产业
    Content Tokens: 百度 展示 驾驶 领域 领导者 大气 风范 发布 名为 apollo 阿波罗 计划 汽车 行业 驾驶 领域 合作 伙伴 提供 开放 完整 软件 平台 车辆 硬件 系统 搭建 完整 驾驶 系统
    LDA sim = 0.0124648
    TWE sim = 0.176046
...
将脚本中的--mode参数修改为1，程序采用长文本与长文本语义匹配计算模式, 运行结果如下所示
    ...
    Doc1 Tokens: 人工智能 发展 系统化 硅谷 ai 工程师 薪水 高于 领域 同行 人工智能 概念 深入人心 人工智能 人才 紧俏 时至今日 大学 毕业 博士 坐拥 八九十 年薪 资深 硅谷 工程师 媲美
    Doc2 Tokens: 国内 企业 瞄准 人才 短板 业界 百度 ai 人才 培养 布局 国内 高校 合作 共建 工程 实验室 数据 开放 资源 共享 上进 合作 方式 类似 美国 人工智能 教育 领域 推行 硅谷 斯坦福 校企 联动 模式 一方 斯坦福 大学 硅谷 提供 人才 科研 成果 一方 硅谷 斯坦福 大学 提供 资金 支持 数据 助力 科研 突破
